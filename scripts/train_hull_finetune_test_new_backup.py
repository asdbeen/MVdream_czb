        ...
        for it, batch in enumerate(epoch_loader):
            ...
            if 'render_image_groundtruth' in batch:
                ...
                for bi in range(bs):
                    ...
                    # Per-view context: each view has its own text, hull, pose
                    n_views = gts.shape[0]
                    # text_c: [n_views, 77, D]
                    text_c = model.get_learned_conditioning([str(category)] * n_views).to(device)
                    # hull_rep: [n_views, 257, D]
                    hull_rep_list = []
                    for k in range(hulls.shape[0]):
                        img = hulls[k].unsqueeze(0)
                        e = image_encoder.encode(img)  # [1, 1, D]
                        hull_rep_list.append(e.squeeze(0))  # [1, D]
                    hull_rep = torch.stack(hull_rep_list, dim=0)  # [n_views, 1, D]
                    ...
                    pose_rep = ref_pose_proj(pose_tokens).unsqueeze(1)  # [n_views, 1, D]
                    # 拼接: [n_views, 77+1+1, D]
                    ...
                    # hull_rep:    [4, 257, 1024]
                    # pose_rep:    [4, 1, 1024]
                    # context_cat: [4, 335, 1024]

                    # print("[DEBUG] text_c shape:", text_c.shape)
                    # print("[DEBUG] hull_rep shape:", hull_rep.shape)
                    # print("[DEBUG] pose_rep shape:", pose_rep.shape)
                    # print("[DEBUG] context_cat shape:", context_cat.shape)
                  
                    # for each target view supervise
                    sample_loss = None

                    # ===== 4-view joint forward =====
                    

                    gt_imgs = gts.to(device)  # [V, 3, H, W]

                    with torch.no_grad():
                        enc_posterior = model.encode_first_stage(gt_imgs)
                        z = model.get_first_stage_encoding(enc_posterior)  # [V, 4, h, w]

                    # IMPORTANT: all views should share the same timestep
                    t_single = torch.randint(0, model.num_timesteps, (1,), device=device).long()
                    t = t_single.repeat(n_views)  # [V]

                    noise = torch.randn_like(z)
                    x_t = model.q_sample(z, t, noise=noise)

                    # build camera tensor for all views
                    camera_list = []
                    ...
                    with amp_ctx():
                        ...
                        hull_mask = hull_masks.to(device)  # [V, 1, H, W]
                        ...
                        hull_loss_val, _, _, _ = hull_criterion(
                            pred_alpha.unsqueeze(1),  # [V, 1, 1, H, W]
                            hull_mask.unsqueeze(1)    # [V, 1, 1, H, W]
                        )

                        sample_loss = diffusion_loss + args.lambda_hull * hull_loss_val
                        print(f"[DEBUG] sample_loss: {sample_loss.item():.6f} (diffusion: {diffusion_loss.item():.6f}, hull: {hull_loss_val.item():.6f})")
