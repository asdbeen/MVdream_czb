...
def _to_gray01(image: np.ndarray, size: Optional[Tuple[int, int]] = None) -> np.ndarray:
    """Convert a numpy image to a single-channel float map in [0, 1]."""
    if image is None:
...
def _make_exceed_overlay(image: np.ndarray, exceed_mask: np.ndarray) -> np.ndarray:
    base = np.asarray(image).astype(np.float32)
    red = np.zeros_like(base)
    red[..., 0] = 255.0
    mask = exceed_mask[..., None].astype(np.float32)
    out = base * (1.0 - 0.65 * mask) + red * (0.65 * mask)
    return np.clip(out, 0, 255).astype(np.uint8)


def _estimate_prediction_depth(
    image: np.ndarray,
    method: str,
    normalize: bool,
) -> np.ndarray:
    ...
    if method == "inverse_luminance":
        ...
        raise ValueError(f"Unknown prediction depth method: {method}")
    return _normalize01(depth) if normalize else np.clip(depth, 0.0, 1.0)


def _compute_depth_exceedance(
    pred_images: List[np.ndarray],
    hull_depth_images: List[np.ndarray],
    pred_depth_method: str,
    compare_direction: str,
    margin: float,
    normalize_depth: bool,
) -> Tuple[List[np.ndarray], List[np.ndarray], List[np.ndarray], np.ndarray, str]:
    """Compare estimated prediction depth with uploaded hull depth maps.

    This is a relative diagnostic: prediction depth is estimated from generated RGB,
    then compared against the provided hull depth image after resizing.
    """
    overlays = []
    pred_depth_vis = []
    hull_depth_vis = []
    ...
    for idx, (pred_img, hull_depth_img) in enumerate(zip(pred_images, hull_depth_images), start=1):
        width, height = pred_img.shape[1], pred_img.shape[0]
        pred_depth = _estimate_prediction_depth(
            pred_img,
            method=pred_depth_method,
            normalize=normalize_depth,
        )
        hull_depth = _to_gray01(hull_depth_img, size=(width, height))
        if normalize_depth:
            hull_depth = _normalize01(hull_depth)

        if compare_direction == "pred_gt_hull":
            exceed = pred_depth > (hull_depth + float(margin))
            diff = np.maximum(pred_depth - hull_depth - float(margin), 0.0)
            direction_label = "pred_depth > hull_depth + margin"
        elif compare_direction == "pred_lt_hull":
            exceed = pred_depth < (hull_depth - float(margin))
            diff = np.maximum(hull_depth - pred_depth - float(margin), 0.0)
            direction_label = "pred_depth < hull_depth - margin"
        else:
            raise ValueError(f"Unknown compare direction: {compare_direction}")

        loss = float(diff.mean())
        ratio = float(exceed.mean())
        max_excess = float(diff.max())
        losses.append(loss)
        ratios.append(ratio)
        overlays.append(_make_exceed_overlay(pred_img, exceed))
        pred_depth_vis.append(_depth_to_rgb(pred_depth))
        hull_depth_vis.append(_depth_to_rgb(hull_depth))
        stats.append(
            f"view {idx}: exceed_ratio={ratio:.4f}, exceed_loss={loss:.6f}, max_excess={max_excess:.4f}"
        )

    overlay_grid = np.concatenate(overlays, axis=1) if overlays else None
    mean_loss = float(np.mean(losses)) if losses else 0.0
    mean_ratio = float(np.mean(ratios)) if ratios else 0.0
    status = (
        f"Depth exceed check ({direction_label}, normalize={normalize_depth}, margin={float(margin):.3f}). "
        f"mean_exceed_ratio={mean_ratio:.4f}, mean_exceed_loss={mean_loss:.6f}\n"
        + "\n".join(stats)
    )
    return overlays, pred_depth_vis, hull_depth_vis, overlay_grid, status
...
def _collect_image_depth_pose_triples(
    images: List[Optional[np.ndarray]],
    depth_images: List[Optional[np.ndarray]],
    pose_files: List[Optional[object]],
    expected_num_refs: int = 4,
) -> Tuple[List[np.ndarray], List[np.ndarray], torch.Tensor]:
    """Collect valid (hull image, depth image, pose) triples and expand them.

    A slot is valid only when hull image, depth image, and pose txt are all uploaded.
    """
    triples = []
    missing = []
    for idx, (image, depth_image, pose_file) in enumerate(zip(images, depth_images, pose_files), start=1):
        if image is None and depth_image is None and pose_file is None:
            continue
        if image is None or depth_image is None or pose_file is None:
            missing.append(idx)
            continue
        pose = _parse_single_pose_txt(pose_file)
        if pose is None:
            raise ValueError(f"Pose {idx} txt is empty or invalid.")
        triples.append((image, depth_image, pose))

    if missing:
        raise ValueError(
            "Each uploaded slot must include hull image, depth image, and pose txt. "
            f"Missing hull/depth/pose at slot(s): {missing}"
        )
    if len(triples) == 0:
        raise ValueError("Please upload at least one hull image, depth image, and pose txt.")

    triples = _expand_to_length(triples, expected_num_refs)
    valid_images = [p[0] for p in triples]
    valid_depth_images = [p[1] for p in triples]
    poses = torch.stack([p[2] for p in triples], dim=0)  # [expected_num_refs, 12]
    return valid_images, valid_depth_images, poses
...
def _load_adapter_weights(model, adapter_ckpt_path: str, lora_rank: int, lora_alpha: float, device: str):
    ...
    depth_cond_proj_state = ckpt.get("depth_cond_proj_state")
    if depth_cond_proj_state is not None:
        weight = depth_cond_proj_state["weight"]
        out_features, in_features = weight.shape
        depth_cond_proj = torch.nn.Linear(in_features, out_features)
        depth_cond_proj.load_state_dict(depth_cond_proj_state)
        depth_cond_proj.to(device)
        depth_cond_proj.eval()
        print("Loaded depth_cond_proj from adapter checkpoint.")
    else:
        print("depth_cond_proj_state not found in checkpoint; using embedding dim auto-match.")

    return cond_proj, depth_cond_proj, ref_pose_proj
...
def _load_depth_encoder_weights(depth_encoder: ImageEmbedder, adapter_ckpt_path: str, device: str) -> None:
    ckpt = torch.load(adapter_ckpt_path, map_location="cpu")
    depth_encoder_state = ckpt.get("depth_encoder_state")
    if depth_encoder_state is not None:
        depth_encoder.load_state_dict(depth_encoder_state)
        depth_encoder.to(device)
        depth_encoder.eval()
        print("Loaded depth_encoder_state from adapter checkpoint.")
    else:
        print("[warn] depth_encoder_state not found in checkpoint; depth conditioning may be weak with this adapter.")
...
def _prepare_depth_embed(
    depth_encoder: ImageEmbedder,
    images: List[np.ndarray],
    image_size: int,
    device: str,
    depth_cond_proj: Optional[torch.nn.Module],
    target_text_dim: int,
) -> torch.Tensor:
    """Encode paired depth images into per-view depth tokens."""
    if len(images) == 0:
        raise ValueError("Please upload at least one depth image.")
    ...
    with torch.no_grad():
        for im in images:
            x = to_tensor(im).unsqueeze(0).to(device)
            emb = depth_encoder.encode(x)
            embeds.append(emb)

    depth_embed = torch.cat(embeds, dim=0)  # (num_images, 1, D)
    if depth_embed.shape[-1] != target_text_dim:
        if depth_cond_proj is not None:
            depth_embed = depth_cond_proj(depth_embed)
        else:
            raise ValueError(
                f"Depth embedding dim {depth_embed.shape[-1]} != text dim {target_text_dim}, "
                "and no depth_cond_proj_state exists in checkpoint."
            )
    return depth_embed
...
def _sample_multiview(
    ...
    with torch.no_grad(), amp_ctx:
        ...
        depth_rep = depth_embed.to(device)
        if depth_rep.shape[0] != num_frames:
            raise ValueError(
                f"Depth embedding batch {depth_rep.shape[0]} != num_frames {num_frames}."
            )
        if ref_pose_embed is not None:
            ...
            context_cat = torch.cat([c_text, hull_rep, depth_rep, pose_rep], dim=1)
            uc_context_cat = torch.cat(
                [uc_text_batch, torch.zeros_like(hull_rep), torch.zeros_like(depth_rep), torch.zeros_like(pose_rep)],
                dim=1,
            )
        else:
            context_cat = torch.cat([c_text, hull_rep, depth_rep], dim=1)
            uc_context_cat = torch.cat([uc_text_batch, torch.zeros_like(hull_rep), torch.zeros_like(depth_rep)], dim=1)
...
def build_app(args):
    ...
    cond_proj, depth_cond_proj, ref_pose_proj = _load_adapter_weights(
        model,
    ...
    image_encoder.eval()
    depth_encoder = ImageEmbedder(device=args.device, img_size=args.size)
    _load_depth_encoder_weights(depth_encoder, adapter_ckpt_path=args.adapter_ckpt, device=args.device)
    depth_encoder.eval()
    ...
    def infer(
        ...
        try:
            ...
            paired_images, paired_depth_images, paired_poses = _collect_image_depth_pose_triples(
            ...
            depth_embed = _prepare_depth_embed(
                depth_encoder=depth_encoder,
                images=paired_depth_images,
                image_size=args.size,
                device=args.device,
                depth_cond_proj=depth_cond_proj,
                target_text_dim=text_dim,
            )
            ...
            exceed_overlays, pred_depth_maps, hull_depth_maps, exceed_grid, exceed_status = _compute_depth_exceedance(
                pred_images=images,
                hull_depth_images=paired_depth_images,
                pred_depth_method=pred_depth_method,
                compare_direction=compare_direction,
                margin=float(depth_margin),
                normalize_depth=bool(normalize_depth),
            )
            status = (
                f"Done. category='{category_name}', paired refs={len(paired_images)}, depth_condition=True\n"
                f"{exceed_status}"
            )
            return grid, images, exceed_grid, exceed_overlays, pred_depth_maps, hull_depth_maps, status
    ...
    with gr.Blocks(title="MVDream Adapter Inference") as demo:
        ...
        with gr.Row():
            with gr.Column(scale=1):
                ...
                gr.Markdown("### Depth Exceed Check")
                pred_depth_method = gr.Radio(
                    choices=[
                        ("Inverse luminance (dark object = large depth)", "inverse_luminance"),
                        ("Luminance (bright = large depth)", "luminance"),
                    ],
                    value="inverse_luminance",
                    label="Prediction Depth Proxy",
                )
                compare_direction = gr.Radio(
                    choices=[
                        ("Prediction depth > hull depth", "pred_gt_hull"),
                        ("Prediction depth < hull depth", "pred_lt_hull"),
                    ],
                    value="pred_gt_hull",
                    label="Exceed Direction",
                )
                depth_margin = gr.Slider(0.0, 0.5, value=0.02, step=0.005, label="Depth Margin")
                normalize_depth = gr.Checkbox(value=True, label="Normalize Depth Per View")
            ...
            with gr.Column(scale=1):
                ...
                exceed_grid_out = gr.Image(type="numpy", label="Depth Exceed Overlay Grid (red=exceeds hull)")
                exceed_gallery_out = gr.Gallery(label="Depth Exceed Overlays", columns=4, object_fit="contain", height=220)
                pred_depth_gallery_out = gr.Gallery(label="Estimated Prediction Depth", columns=4, object_fit="contain", height=220)
                hull_depth_gallery_out = gr.Gallery(label="Hull Depth Used for Loss", columns=4, object_fit="contain", height=220)
        ...
        run_btn.click(
            infer,
            inputs=[image1, depth1, pose1_txt, image2, depth2, pose2_txt, image3, depth3, pose3_txt, image4, depth4, pose4_txt,
                    cat_entity, cat_volume, cat_direction, cat_operation, cat_affect,
                    prompt, negative_prompt, steps, guidance_scale, seed,
                    pred_depth_method, compare_direction, depth_margin, normalize_depth],
            outputs=[
                grid_out,
                gallery_out,
                exceed_grid_out,
                exceed_gallery_out,
                pred_depth_gallery_out,
                hull_depth_gallery_out,
                status,
            ],
        )

        def _save_gallery_images(items, tmpdir, prefix):
            paths = []
            if not items:
                return paths
            for i, item in enumerate(items):
                im = item[0] if isinstance(item, (tuple, list)) else item
                if im is None:
                    continue
                if isinstance(im, np.ndarray):
                    ...
                    img = im
                p = os.path.join(tmpdir, f"{prefix}_{i+1}.png")
                img.save(p)
                paths.append(p)
            return paths

        def _make_download(grid, images, exceed_grid, exceed_images, pred_depth_images, hull_depth_images):
            ...
            try:
                ...
                if exceed_grid is not None:
                    exceed_grid_img = Image.fromarray(exceed_grid.astype(np.uint8))
                    exceed_grid_img.save(os.path.join(tmpdir, "depth_exceed_overlay_grid.png"))

                # Save individual views
                img_paths = _save_gallery_images(images, tmpdir, "view")
                _save_gallery_images(exceed_images, tmpdir, "depth_exceed_overlay")
                _save_gallery_images(pred_depth_images, tmpdir, "pred_depth")
                _save_gallery_images(hull_depth_images, tmpdir, "hull_depth")
