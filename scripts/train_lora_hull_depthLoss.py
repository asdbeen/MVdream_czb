...
def normalize_depth_per_image(depth: torch.Tensor, eps: float = 1e-8) -> torch.Tensor:
    """Normalize [B,1,H,W] depth maps to [0,1] per image while keeping gradients."""
...
def depth_exceed_loss_from_rgb(
    ...
    """Differentiable depth exceed loss.

    decoded_imgs: [B,3,H,W] generated prediction in [0,1]
    hull_depth:   [B,1,H,W] or [B,3,H,W] hull depth in [0,1]
    """
...
def train(args):
    ...
    if args.dataset_root is not None and args.meta_path is not None:
        ...
        if args.val_meta_path is not None:
            ...
            val_dataset = DepthConditionDataset(
                val_dataset,
                root_dir=args.dataset_root,
                depth_dir_name=args.depth_dir_name,
                image_size=args.size,
    ...
    if args.resume_ckpt:
        ...
        if "image_encoder_state" in resume:
            ...
            image_encoder.to(device)
            image_encoder.backbone.eval()
        if "depth_encoder_state" in resume:
            depth_encoder.load_state_dict(resume["depth_encoder_state"])
            depth_encoder.to(device)
            depth_encoder.backbone.eval()
        if cond_proj is not None and "cond_proj_state" in resume:
            cond_proj.load_state_dict(resume["cond_proj_state"])
        if depth_cond_proj is not None and "depth_cond_proj_state" in resume:
            depth_cond_proj.load_state_dict(resume["depth_cond_proj_state"])
    ...
    # Validation function
    def validate(val_loader, model, sampler, image_encoder, depth_encoder, cond_proj, depth_cond_proj, ref_pose_proj, device, fp16, save_images=False, save_dir=None):
        ...
        with torch.no_grad():
            for batch in val_loader:
                ...
                with torch.autocast(device_type='cuda', dtype=torch.float16, enabled=fp16):
                    ...
                    # Depth embedding
                    depth_emb = depth_encoder(depths)
                    if depth_cond_proj is not None:
                        depth_emb = depth_cond_proj(depth_emb)
                    ...
                    depth_rep = depth_emb.unsqueeze(1)
                    pose_rep = pose_emb.unsqueeze(1)
                    c = torch.cat([c_text, hull_rep, depth_rep, pose_rep], dim=1)
    ...
    for epoch in range(start_epoch, args.epochs):
        ...
        for it, batch in enumerate(epoch_loader):
            ...
            if 'render_image_groundtruth' in batch:
                ...
                with amp_ctx():
                    ...
                    depth_targets = depths.view(B, V, *depths.shape[1:])
                    ...
                    # [B, 3, H, W], selected hull depth target for the same view
                    hull_depth = depth_targets[batch_ids, rand_views].float()
                    ...
                    with torch.cuda.amp.autocast():
                        ...
                        depth_exceed_loss, depth_exceed_map, pred_depth, hull_depth_for_loss = depth_exceed_loss_from_rgb(
                    ...
                    if args.save_pred_images and (target_save_epochs is None or (epoch + 1) in target_save_epochs):
                        ...
                        for idx in range(max_save):
                            ...
                            pred_depth_path = os.path.join(pred_image_dir, f'epoch{epoch+1}_iter{it}_preddepth_{idx}.png')
                            _save_pred_image(pred_depth[idx], pred_depth_path)
                            hull_depth_path = os.path.join(pred_image_dir, f'epoch{epoch+1}_iter{it}_hulldepth_{idx}.png')
                            _save_pred_image(hull_depth_for_loss[idx], hull_depth_path)
                            depth_exceed_path = os.path.join(pred_image_dir, f'epoch{epoch+1}_iter{it}_depthexceed_{idx}.png')
                            _save_pred_image(depth_exceed_map[idx], depth_exceed_path)
                            print(f"[LOG] Saved predalpha: {pred_alpha_path}")
                    total_loss = diffusion_loss + args.lambda_hull * outside_loss + args.lambda_depth_hull * depth_exceed_loss
                    print(
                        f"[LOG] epoch={epoch+1} iter={it} total_loss={total_loss.item():.6f} "
                        f"diffusion_loss={diffusion_loss.item():.6f} outside_loss={outside_loss.item():.6f} "
                        f"depth_exceed_loss={depth_exceed_loss.item():.6f}"
                    )
        ...
        if val_dl is not None:
            ...
            val_loss = validate(val_dl, model, sampler, image_encoder, depth_encoder, cond_proj, depth_cond_proj, ref_pose_proj, device, args.fp16, 
...
if __name__ == '__main__':
    ...
    parser.add_argument('--depth_dir_name', type=str, default='depth_hull', help='per-sample directory containing depth PNGs with matching view ids')
    ...
    parser.add_argument('--lambda_depth_hull', type=float, default=1.0, help='weight for depth exceed hull loss')
    parser.add_argument(
        '--pred_depth_method',
        type=str,
        default='inverse_luminance',
        choices=['inverse_luminance', 'luminance'],
        help='how to estimate prediction depth from decoded RGB',
    )
    parser.add_argument(
        '--depth_exceed_direction',
        type=str,
        default='pred_gt_hull',
        choices=['pred_gt_hull', 'pred_lt_hull'],
        help='which depth relation is considered exceeding the hull',
    )
    parser.add_argument('--depth_margin', type=float, default=0.02, help='margin before depth exceed loss is applied')
    parser.add_argument('--normalize_depth_loss', action=argparse.BooleanOptionalAction, default=True, help='normalize prediction and hull depth per image before depth exceed loss')
